...
1. Verify `/usr/share/dict/words` exists.  If it does not, see if `/usr/dict/words` exists.
2. For the `grep` command, describe what the `-c` and `-i` options do. (1 pt)
3. For each below, write the `grep` command you used as well as the answer.
...
2. Add the folder these scripts are located in to your `PATH`.  (2 pts)
...
3. When the command / script `marco` is run, it should save the current working directory in some manner.  Copy your working `marco` script into your lab notes. (1 pt)
4. When the command / script `polo` is run, it should `cd` you back to the directory where you executed `marco`.  Copy your working `polo` script into your lab notes. (1 pt)
